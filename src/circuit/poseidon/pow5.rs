...
use super::primitives::{Absorbing, Domain, Mds, Spec, Squeezing};

use crate::circuit::poseidon::base::{
    PaddedWord, PoseidonInstructions, PoseidonSpongeInstructions, State,
};
    ...
    PoseidonInstructions<F, S, WIDTH, RATE> for Pow5Chip<F, WIDTH, RATE>
{
    ...
    fn permute(
        ...
        let state = Pow5State::load(region, config, initial_state)?;

        let state = (0..config.half_full_rounds).fold(Ok(state), |res, r| {
            *offset = r;
            res.and_then(|state| state.full_round(region, config, r, offset))
        })?;

        let state = (0..config.half_partial_rounds).fold(Ok(state), |res, r| {
            res.and_then(|state| {
                *offset = config.half_full_rounds + r;
                state.partial_round(region, config, config.half_full_rounds + 2 * r, offset)
            })
        })?;

        let state = (0..config.half_full_rounds).fold(Ok(state), |res, r| {
            res.and_then(|state| {
                *offset = config.half_full_rounds + config.half_partial_rounds + r;
                state.full_round(
                    region,
                    config,
    ...
    > PoseidonSpongeInstructions<F, S, D, WIDTH, RATE> for Pow5Chip<F, WIDTH, RATE>
{
    ...
    fn add_input(
        ...
        config.s_pad_and_add.enable(region, 1)?;
        ...
        let load_input_word = |i: usize| {
            ...
            constraint_var
                .copy_advice(|| format!("load input_{}", i), region, config.state[i], 1)
                .map(StateWord)
        };
...
mod tests {
    ...
    use crate::circuit::poseidon::spec::PoseidonSpec;

    use crate::circuit::poseidon::base::Hash;
    use crate::circuit::poseidon::primitives::{self as poseidon, ConstantLength, Spec};
        ...
        fn synthesize(
            ...
            layouter.assign_region(
                ...
                |mut region| {
                    ...
                    let initial_state = state?.try_into().unwrap();
                    ...
                    let final_state = <Pow5Chip<_, WIDTH, RATE> as PoseidonInstructions<
                        ...
                        RATE,
                    >>::permute(
                        &chip, &mut region, &initial_state, &mut offset
                    )?;
    ...
    fn poseidon_permute() {
        let k = 6;
        let circuit = PermuteCircuit::<PoseidonSpec, 15, 14>(PhantomData);
        ...
        fn synthesize(
            ...
            layouter.assign_region(
                ...
                |mut region| {
                    ...
                    let message = message?.try_into().unwrap();
                    ...
                    let output = hasher.hash(&mut region, message, &mut offset)?;
    ...
    fn poseidon_hash() {
        ...
        let output =
            poseidon::Hash::<_, PoseidonSpec, ConstantLength<2>, 15, 14>::init().hash(message);

        let k = 6;
        let circuit = HashCircuit::<PoseidonSpec, 15, 14, 2> {
    ...
    fn poseidon_hash_longer_input() {
        ...
        let output =
            poseidon::Hash::<_, PoseidonSpec, ConstantLength<3>, 15, 14>::init().hash(message);

        let k = 7;
        let circuit = HashCircuit::<PoseidonSpec, 15, 14, 3> {
            message: Value::known(message),
            output: Value::known(output),
            _spec: PhantomData,
        };
