...
impl<F: PrimeField + TensorType + PartialOrd + std::hash::Hash> Table<F> {
    ...
    pub fn layout(
        ...
        eprintln!("[ezkl] layout table: {} (range {}..{})", self.name(), self.range.0, self.largest());

        let smallest = self.range.0;
        let largest = self.largest();

        // Table content is generated by nonlinearity.f(): for built-in ops this evaluates
        // the function over the range; for LookupOp::Custom it loads PWL data from the
        // file path and applies it. No special initialization is needed for Custom—the
        // same TableColumn fill path is used for all LookupOp variants.
        ...
        let (inputs, evals) = if !LOOKUP_CACHE.is_empty() {
            ...
            if matches!(self.nonlinearity, LookupOp::Custom { .. }) {
                let path_str = match &self.nonlinearity {
                    LookupOp::Custom { path, .. } => path.as_str(),
                    _ => "",
                };
                eprintln!(
                    "[ezkl] custom lookup: generating table from {} (range {}..{}) ...",
                    path_str,
                    smallest,
                    largest
                );
                log::info!(
                    "Generating custom lookup table from path: {} (range [{}, {}])",
                    path_str,
                    smallest,
                    largest
                );
        ...
        if matches!(self.nonlinearity, LookupOp::Custom { .. }) {
            eprintln!("[ezkl] custom lookup: table generated, assigning cells ...");
        }
        ...
        let _ = chunked_inputs
            ...
            .collect::<Result<Vec<()>, halo2_proofs::plonk::Error>>()?;
        if matches!(self.nonlinearity, LookupOp::Custom { .. }) {
            eprintln!("[ezkl] custom lookup: layout done.");
        }
        Ok(())
    }
}
