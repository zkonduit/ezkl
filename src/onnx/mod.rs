...
impl OnnxNode {
    ...
    //     // should match on Op, e.g. Const( or Gemm (, but these are tough to access
    //     match self.opkind() {
    //         OpKind::Affine => Ok(OnnxNodeConfig::Affine(Affine1dConfig::configure(
    //             meta, params, input, output,
    //         ))),
    //         OpKind::Convolution => Ok(OnnxNodeConfig::Conv(ConvConfig::configure(
    //             meta, params, input, output,
    //         ))),
    //         OpKind::ReLU => {
    //             // we want to check: assert_eq!(self.node.op(),tract_hir::ops::activations::Clip::new(Some(0.0), None));
    //             let relu: EltwiseConfig<F, BITS, ReLu<F>> = EltwiseConfig::configure(
    //                 meta, input, None, //todo, check for previous copy
    //             );

    //             Ok(OnnxNodeConfig::ReLU(relu))
    //         }
    //         _ => todo!(),
    //     }
    // }
