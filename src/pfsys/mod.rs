...
impl<F: FieldExt> Proof<F> {
    ...
    pub fn load(proof_path: &PathBuf) -> Result<Self, Box<dyn Error>> {
        ...
        file.read_to_string(&mut data)
            .map_err(Box::<dyn Error>::from)?;
        let proof_i32: Proofi32 = serde_json::from_str(&data).map_err(Box::<dyn Error>::from)?;
        Ok(Proof {
            public_inputs: proof_i32
                .public_inputs
                .iter()
...
>(
    ...
    strategy: Strategy,
) -> Result<Proof<Scheme::Scalar>, halo2_proofs::plonk::Error>
where
    C: Circuit<Scheme::Scalar>,
{
    let now = Instant::now();
