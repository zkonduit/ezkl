...
impl<F: FieldExt, C: CurveAffine> Snark<F, C> {
    ...
    pub fn new(protocol: Option<PlonkProtocol<C>>, instances: Vec<Vec<F>>, proof: Vec<u8>) -> Self {
        Self {
            protocol,
            instances,
            proof,
    ...
    pub fn load<Scheme: CommitmentScheme<Curve = C, Scalar = F>>(
        ...
        if params.is_none() || vk.is_none() {
            ...
            let protocol = compile(
                params.unwrap(),
                vk.unwrap(),
                Config::kzg().with_num_instance(proof_i32.num_instance),
            );
