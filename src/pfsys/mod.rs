...
impl<F: PrimeField + SerdeObject + FromUniformBytes<64> + Serialize, C: CurveAffine> Snark<F, C> {
    ...
    pub fn save(&self, proof_path: &PathBuf) -> Result<(), Box<dyn Error>> {
        ...
        serde_json::to_writer(&mut writer, &self_i128)?;
        Ok(())
    }
        ...
        <C as CurveAffine>::ScalarExt: FromUniformBytes<64>,
    {
        ...
        let snark_bytes: Snarkbytes = serde_json::from_str(&data)?;
