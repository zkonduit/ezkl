        ...
        * ((IMAGE_WIDTH + 2 * PADDING - KERNEL_WIDTH) / STRIDE + 1)]:,
{
    ...
    fn synthesize(
        ...
        let l1out = config.l1.layout(
            &mut layouter,
            self.l1_params.0.clone(),
            self.l1_params.1.clone(),
            l0out.clone().into_iter().flatten().flatten().collect(),
        )?;
        //        println!("l1out {:?}", l1out);
        println!(
            "{:?}",
        ...
        for i in 0..LEN {
            //            layouter.constrain_instance(l1out[i].cell(), config.public_output, i)?;
        }
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_useconv() {
        ...
        let l1_params: (Vec<Vec<i32>>, Vec<i32>) = (l2weights, l2biases);
        ...
        let public_input = vec![-9, -15, 1, -19, 7, -18, -6, -8, -19];
