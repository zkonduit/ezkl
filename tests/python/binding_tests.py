...
def test_forward():
    ...
    assert data["processed_inputs"] == res["processed_inputs"] == {'poseidon_hash': [
        [8270957937025516140, 11801026918842104328, 2203849898884507041, 140307258138425306]]}
    assert data["processed_params"] == {
        'poseidon_hash': []} == res["processed_params"]
    assert data["processed_outputs"] == res["processed_outputs"] == {'poseidon_hash': [
        [4554067273356176515, 2525802612124249168, 5413776662459769622, 1194961624936436872]]}
