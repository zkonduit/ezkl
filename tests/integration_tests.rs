        ...
        mod tests_evm {
            ...
            seq!(N in 0..=22 {
                ...
                fn kzg_evm_prove_and_verify_render_seperately_(test: &str) {
                    ...
                    let path = test_dir.path().to_str().unwrap(); crate::native_tests::mv_test_(path, test);
                    let _anvil_child = crate::native_tests::start_anvil(false, Hardfork::Latest);
                    kzg_evm_prove_and_verify_render_seperately(2, path, test.to_string(), "private", "private", "public",false);
                    // #[cfg(not(feature = "icicle"))]
                    // run_js_tests(path, test.to_string(), "testBrowserEvmVerify");
                    test_dir.close().unwrap();
                ...
                #(#[test_case(TESTS_EVM[N])])*
                #[serial]
                fn kzg_evm_prove_and_verify_render_seperately_universal_(test: &str) {
                    crate::native_tests::init_binary();
                    let test_dir = TempDir::new(test).unwrap();
                    let path = test_dir.path().to_str().unwrap();
                    crate::native_tests::mv_test_(path, test);
                    ...
                    if test == TESTS_EVM[0] {
                        kzg_evm_prove_and_verify_render_seperately(2, path, test.to_string(), "private", "private", "public", false);
                        // cp  the universal verifier from test_dir/addr.text to /tmp/ezkl-tests/universal-verifier/test
                        let mut universal_verifier_addr = std::fs::File::open(format!("{}/{}/addr.txt", path, test)).unwrap();
                        let mut universal_verifier_contents = String::new();
                        universal_verifier_addr.read_to_string(&mut universal_verifier_contents).unwrap();
                        std::fs::create_dir_all("/tmp/ezkl-tests/universal-verifier").unwrap();
                        let mut universal_verifier_addr = std::fs::File::create("/tmp/ezkl-tests/universal-verifier/test").unwrap();
                        println!("universal_verifier_addr: {:?}", universal_verifier_addr);
                        universal_verifier_addr.write_all(universal_verifier_contents.as_bytes()).unwrap();
                    } else
                    {
                        // cp the universal verifier generated by the first test to the other tests
                        let mut universal_verifier_addr = std::fs::File::open("/tmp/ezkl-tests/universal-verifier/test").unwrap();
                        let mut universal_verifier_contents = String::new();
                        universal_verifier_addr.read_to_string(&mut universal_verifier_contents).unwrap();
                        let mut universal_verifier_addr = std::fs::File::create(format!("{}/{}/addr.txt", path, test)).unwrap();
                        universal_verifier_addr.write_all(universal_verifier_contents.as_bytes()).unwrap();

                        kzg_evm_prove_and_verify_render_seperately(2, path, test.to_string(), "private", "private", "public", true);
                    }
                    // #[cfg(not(feature = "icicle"))]
                    // run_js_tests(path, test.to_string(), "testBrowserEvmVerify");
                    test_dir.close().unwrap();
